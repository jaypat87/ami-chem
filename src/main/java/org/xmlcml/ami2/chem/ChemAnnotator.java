...
public class ChemAnnotator {
	...
	private static String HIDE_CHARACTER = "??????????";
	...
	private static void createMoleculeLabels(MoleculeCreator cmlCreator, String name, HtmlBody body) {
		int i = 0;
		for (Entry<Real2Range, Map<Real2Range, CMLMolecule>> mol : cmlCreator.moleculeLocations.columnMap().entrySet()) {
			CMLMolecule molecule = MoleculeCreator.getMolecule(mol);
			HtmlDiv div = new HtmlDiv();
			body.appendChild(div);
			div.setId("molecule" + i);
			double left = mol.getKey().getXMin() * 2;
			double top = mol.getKey().getYMin() * 2 - 15;
			...
			div.appendChild(" ");
			if (molecule.getLabelElements().get(0) != null) {
				addLink(div, "http://www.ncbi.nlm.nih.gov/pccompound?term=" + molecule.getLabelElements().get(0).getCMLValue().replace("^", "").replace("_", ""), "PubChem");
				div.appendChild(" ");
			}
